...
class EventListener(object):
    def __init__(self, on_event=None, white_list=None, black_list=[], **kwargs):
        self.white_list = [white_list] if isinstance(white_list, (basestring, re.Pattern)) else white_list
        self.black_list = [black_list] if isinstance(black_list, (basestring, re.Pattern)) else black_list
    ...
    def check_white_list(self, event_name):
        ...
        for rule in self.white_list:
            if isinstance(rule, basestring) and event_name == rule:
                return True
            if isinstance(rule, re.Pattern) and rule.search(event_name) is not None:
                return True
        return False
    ...
    def check_black_list(self, event_name):
        for rule in self.black_list:
            if isinstance(rule, basestring) and event_name == rule:
                return False
            if isinstance(rule, re.Pattern) and rule.match(event_name) is not None:
                return False
        return True

    def check_attribute(self, rules, value):
        if isinstance(rules, (re.Pattern, basestring)):
            rules = [rules]
        for rule in rules:
            if isinstance(rule, basestring) and rule == value:
                return True
            if isinstance(rule, re.Pattern) and rule.search(value):
                return True
        return False
